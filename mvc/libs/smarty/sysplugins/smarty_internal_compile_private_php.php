...
class Smarty_Internal_Compile_Private_Php extends Smarty_Internal_CompileBase
{
     ...
     */
    public $required_attributes = array('code.class', 'type');
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter)
    {
        ...
        if ($_attr[ 'type' ] == 'xml') {
            ...
            $output = addcslashes($_attr['code.class'], "'\\");
        ...
        if ($_attr[ 'type' ] != 'tag') {
            if ($compiler->php_handling == Smarty::PHP_REMOVE) {
                ...
                $output =
                    preg_replace_callback('#(<\?(?:php|=)?)|(<%)|(<script\s+language\s*=\s*["\']?\s*php\s*["\']?\s*>)|(\?>)|(%>)|(<\/script>)#i',
                                          array($this, 'quote'), $_attr['code.class']);
                $compiler->parser->current_buffer->append_subtree($compiler->parser,
            ...
            preg_match("#^({$ldel}php\\s*)((.)*?)({$rdel})#", $_attr['code.class'], $match);
            if (!empty($match[ 2 ])) {
